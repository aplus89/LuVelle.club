...
const paymentIntentOptions = [
  "Sí, si me ayuda a recuperar citas o clientas",
  "Tal vez, depende de cuánto valor me genere",
  "Prefiero probarla antes de decidir",
  "No por ahora",
]
...
export function AiProviderExperiment() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setLoading(true)
    const result = await createLuvelleAiInterestAction({
      full_name: formData.name,
      email: formData.email,
      whatsapp: formData.whatsapp || undefined,
      business_type: formData.businessType || undefined,
      biggest_pain_point: formData.mainPain,
      weekly_inquiries_estimate: formData.weeklyInquiries || undefined,
      current_followup_method: formData.followupMethod || undefined,
      would_pay_if_it_recovers_clients: formData.paymentIntent || undefined,
      notes: formData.notes || undefined,
      source: "luvelle-ai-beta-v1",
    })
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="rounded-[28px] border border-[#E9DDF2] bg-white p-6 shadow-[0_18px_60px_rgba(91,42,134,0.08)] md:p-8">
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <div><Label htmlFor="ai-business-type">Especialidad / tipo de negocio</Label><Input id="ai-business-type" placeholder="Ej. pestañas, uñas, estética..." className="mt-2" value={formData.businessType} onChange={(e) => setFormData({ ...formData, businessType: e.target.value })} /></div>
                ...
                <div className="md:col-span-2"><Label htmlFor="ai-payment-intent">Si LuVelle AI te ayudara a recuperar citas o clientas, ¿pagarías por usarla?</Label><select id="ai-payment-intent" value={formData.paymentIntent} onChange={(e) => setFormData({ ...formData, paymentIntent: e.target.value })} className="mt-2 h-11 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="">Seleccioná una opción</option>{paymentIntentOptions.map((option) => <option key={option}>{option}</option>)}</select></div>
                <div className="md:col-span-2"><Label htmlFor="ai-notes">¿Qué tendría que hacer LuVelle AI para convertirse en una herramienta valiosa para tu negocio?</Label><Textarea id="ai-notes" className="mt-2 min-h-24" value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} /></div>
              </div>
